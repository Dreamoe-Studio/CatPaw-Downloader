...
class DownloaderApp:
    def __init__(self, root):
        ...
        # 获取系统信息
        os_name = f"{platform.system()} {platform.release()}"  # 操作系统名称
        architecture = platform.machine()  # 系统架构
        kernel_version = platform.version()  # 内核版本
        
        # 设置窗口标题
        self.root.title(f"离线更新包下载器   下载器版本号: v{CURRENT_VERSION} ({CURRENT_VER_CODE})   系统版本: {os_name} 系统架构: {architecture} 内核版本: {kernel_version}")
